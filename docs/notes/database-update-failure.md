...
After this, it was possible that there was an issue with that version of the `pgcrypto`, but after checking this, it was dicovered that this was the laterst version of the library (outside of a small update to another function in pg18, which had only just been released)
...
From the above, the issue is with the location of the `digest`function during the upgrade, so there are a few ways around this which are as follows:

- Drop and recreate the  after update
  - This would cause the function to not exist during the update
  - Potential issues with data loss
...
- Replace the `digest` function with an inbuilt function
  - Best possible fix as it saves manual intervention etc.
