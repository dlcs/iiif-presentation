...
public class ModifyCollectionTests : IClassFixture<PresentationAppFactory<Program>>
{
    ...
    [Fact]
    public async Task CreateCollection_ReturnsConflict_WhenOnlyCaseDiffers()
    {
        // Arrange
        var collection = new UpsertFlatCollection
        {
        ...
        var secondCollection = new UpsertFlatCollection
        {
            Behavior = new()
            {
                Behavior.IsPublic,
            ...
            Label = new("en", ["test collection"]),
            Slug = "PrOgRaMmAtIc-ChIlD", // note just casing difference
            Parent = parent,
            PresentationThumbnail = "some/thumbnail",
            Tags = "some, tags",
            ItemsOrder = 1
        ...
        var requestMessageOne = HttpRequestMessageBuilder.GetPrivateRequest(HttpMethod.Post, $"{Customer}/collections",
            JsonSerializer.Serialize(collection));

        var requestMessageTwo = HttpRequestMessageBuilder.GetPrivateRequest(HttpMethod.Post, $"{Customer}/collections",
            JsonSerializer.Serialize(secondCollection));

        // Act
        var response = await httpClient.AsCustomer(1).SendAsync(requestMessageOne);
        var secondResponse = await httpClient.AsCustomer(1).SendAsync(requestMessageTwo);
        ...
        // The previous worked fine, as expected, now verify the latter did not:
